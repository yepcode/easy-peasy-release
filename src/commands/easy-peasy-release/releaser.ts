...
export default abstract class Releaser extends Command {
  ...
  async run(): Promise<void> {
    const { args } = await this.parse(Releaser)
    ...
    const tasks = new TaskRenderer([
      {
        title: color.bold(`bumping version in ${color.gray('package.json')} from ${color.cyan(currentVersion)} to ${color.cyan(nextVersion)}`),
        task: () => this.updateVersion(nextVersion),
      },
      {
        title: color.bold(`commiting ${color.gray('package.json')}`),
        task: async () => {
          await git.add('package*')
          return git.commit(`Release ${versionLabel}`)
        },
      },
      {
        title: color.bold(`tagging release ${color.bold(versionLabel)}`),
        task: async (ctx) => {
          await git.tag(['-a', versionLabel, '-m', `Bump version to ${nextVersion}`])
          ctx.tagInfo = await git.show([versionLabel])
          return ctx.tagInfo
        },
      },
    ])
    ...
    if (pushToRemote) {
      await git.push(['--follow-tags', 'origin', currentBranch])
